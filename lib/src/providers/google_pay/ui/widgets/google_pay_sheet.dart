/// Google Pay payment sheet with comprehensive UI.
library;
...
/// Google Pay payment sheet modal.
class GooglePaySheet extends StatefulWidget {
  ...
  @override
  State<GooglePaySheet> createState() => _GooglePaySheetState();
}

class _GooglePaySheetState extends State<GooglePaySheet>
  ...
  Future<void> _checkGooglePayAvailability() async {
    ...
    if (mounted) {
      setState(() {
        _availability = const GooglePayAvailability(
          isAvailable: true,
          supportedNetworks: [CardNetwork.visa, CardNetwork.mastercard],
        );
      });
    }
  }
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  widget.title ?? 'Pay with Google Pay',
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          if (widget.paymentRequest.label != null) ...[
            const SizedBox(height: 8.0),
            _buildDetailRow('Description', widget.paymentRequest.label!, theme),
          ],
        ],
      ),
    );
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 8.0),
          Text(
            _availability?.reason ?? 'Google Pay is not available on this device.',
            style: theme.textTheme.bodyMedium?.copyWith(
              color: theme.colorScheme.onSurfaceVariant,
            ),
...
/// Extension to add copyWith method to GooglePayButtonConfig.
extension GooglePayButtonConfigExtension on GooglePayButtonConfig {
  GooglePayButtonConfig copyWith({
